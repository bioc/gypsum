...
#' This can be the name of any SQLite file in \url{https://github.com/ArtifactDB/gypsum-to-sqlite/releases/tag/latest},
#' after removing the \code{.sqlite3} suffix.
...
fetchMetadataDatabase <- function(name="bioconductor", cache=cacheDirectory(), overwrite=FALSE) {
    base.url <- "https://github.com/ArtifactDB/gypsum-to-sqlite/releases/download/latest/"
    ...
    if (is.null(cache)) {
        ...
        cache.path <- file.path(cache.dir, paste0(name, ".sqlite3"))
    ...
    url <- paste0(base.url, name, ".sqlite3")
    req <- request(url)
